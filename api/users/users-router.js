...
router.post('/:id/plants', (req, res) => {
  Users.findUsersPlants(req.params.id)
    .then((plant) => {
      if (plant) {
        Users.insertPlant(req.body, req.params.id).then((newPlant) => {
...
router.put('/:id', (req, res) => {
  Users.update(req.params.id, req.body)
    .then((users) => {
      Users.findBy({ user_id: req.params.id }).then((user) => {
...
router.put('/:id/plants/:id', (req, res) => {
  Users.findUsersPlants(req.params.id)
    .then((plant) => {
      if (plant) {
        Users.updateUsersPlant(req.params.id, req.body).then((newPlant) => {
          res.status(201).json(newPlant);
        });
      }
    })
...
router.delete('/:id/plants/:id', (req, res) => {
  Users.findUsersPlants(req.params.id)
    .then((recipe) => {
      if (recipe) {
        Users.removeUserPlant(req.params.id).then((newPlant) => {
          res.status(201).json(newPlant);
        });
      }
    })
